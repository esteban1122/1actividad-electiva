...
export const createNote: CreateNote = async () => {
  ...
  const normalizedRoot = path.resolve(path.normalize(rootDir))
  const normalizedParent = path.resolve(path.normalize(parentDir))

  if (normalizedRoot !== normalizedParent) {
    await dialog.showMessageBox({
      type: 'error',
      title: 'creation',
      message: `All notes must be saved under:\n${normalizedRoot}\n\nAvoid using other directories!`
    })
    return false
  }
